...
実装課題には特に指定がなかったので、DDD + オニオンアーキテクチャ で実装しています。
ただし厳密なものではなく、Laravelに任せれる部分は任せています。
UsecaseやDomainのクラス群は、laravel/packages/ 以下に配置しています。

### テスト

APIごとに、以下のテストのみ実装しています。
...
バリデーションエラーメッセージを汎用で定義した関係で、一部のメッセージ内容が仕様とズレてます（すみません）
